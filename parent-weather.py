...
def get_weather(city, key):
    url = 'http://v.juhe.cn/weather/index?cityname={}&key={}'.format(city, key)  # 城市名cityname和key值换成自己的
...
if True:
    try:
        config = ConfigWeatherWeChat()
        for i in range(len(config.group_city_name)):
            city = config.group_city_name[i]
            group_name = config.group_name[i]
            temperature, weather, week, city, dressing_advice = get_weather(city, config.key)
            # 发送到微信群里
            my_groups = bot.groups().search(group_name)[0]   # 换成发送信息的群名称
            msg = '今天是：' + week + '\n' \
                  + city + '的天气：' + weather + '\n' \
                  + '今天温度：' + temperature +'\n' \
        ...
        # 单独私发微信
        for i in range(len(config.friend_city_name)):
            city = config.friend_city_name[i]
            friend = config.friend_name[i]
            my_friend = bot.friends().search(friend)[0]  # 此处是对方自己的昵称，不是微信号，也不是你的备注。
